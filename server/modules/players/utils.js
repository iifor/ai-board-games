...
function rowToPlayer(row) {
  if (!row) return null;
  return {
    id: row.id, nickname: row.nickname, name: row.name, avatar: row.avatar,
    sex: row.sex, personality: row.personality, provider: row.provider, model: row.model,
    modelId: row.model_id, voicePackageId: row.voice_package_id, temperature: row.temperature,
    enabled: Boolean(row.enabled), sortOrder: row.sort_order, createdAt: row.created_at, updatedAt: row.updated_at
  };
}

module.exports = { playerToRow, rowToPlayer };
