...
const { processClaimedAiTask } = require('./aiTaskWorker');
...
function enqueueAiTask(task) {
  repo.createAiTask(task);
  return repo.getAiTask(task.id);
}
...
function completeAiTask(taskId, result) {
  ...
  if (!payload || (typeof payload === 'object' && !Object.keys(payload).length)) {
    return failAiTask(taskId, { message: 'AI task result payload is empty', severity: 'high' });
  }
  repo.updateAiTask(task.id, {
    status: 'succeeded',
    raw_output: typeof result.rawOutput === 'string' ? result.rawOutput : JSON.stringify(result.rawOutput ?? payload),
...
function failAiTask(taskId, error = {}) {
  const task = repo.getAiTask(taskId);
  if (!task) throw new Error(`AI task not found: ${taskId}`);
  ...
  return shouldPause ? repo.getMatch(task.matchId) : repo.getAiTask(task.id);
}
...
function cancelAiTask(taskId, reason = 'cancelled') {
...
function manualCompleteAiTask(taskId, payload = {}) {
...
function submitPendingAction({ matchId, actionId, payload = {}, idempotencyKey = '' }) {
  getDb().transaction(() => {
    const match = repo.getMatch(matchId);
    if (!match) throw new Error(`Match not found: ${matchId}`);
    if ([MATCH_STATUS.COMPLETED, MATCH_STATUS.FAILED, MATCH_STATUS.PAUSED_DEBUG].includes(match.status)) {
...
function commitWorkflowChange(change) {
