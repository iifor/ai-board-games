...
async function runWerewolfAiAction(runtime, round, actor, actionType) {
...
async function runActionWindowAiTask({ match, step, task }) {
  const runtime = createRuntime(repo.getMatch(match.id) || match);
...
async function runHunterAiTask({ match, step, task }) {
  const runtime = createRuntime(repo.getMatch(match.id) || match);
  const actor = runtime.agents.find((agent) => Number(agent.id) === Number(task.playerId));
...
function validateActionWindowAiResult({ result }) {
...
async function runDaySpeechAction(runtime, round, actor) {
  const text = actor.thinkingEnabled && actor.playerAgent.thinkingEnabled
...
async function runDayVoteAction(runtime, actor, alive) {
...
function runRoleSkill(runtime, action, context) {
