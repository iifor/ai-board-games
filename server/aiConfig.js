...
function normalizeProvider(name, rawProviders = {}) {
  ...
  const legacyOpenAIBaseUrl = name === 'openai' ? process.env.OPENAI_BASE_URL : '';
  const baseUrl = provider.baseUrl || legacyOpenAIBaseUrl || BUILTIN_PROVIDERS[name]?.baseUrl;
...
function getUsedProviderNames(rawHost, rawPlayers) {
  return Array.from(new Set([
    rawHost.provider || DEFAULT_CONFIG.host.provider,
    ...rawPlayers.map((player) => player.provider || rawHost.provider || DEFAULT_CONFIG.host.provider)
  ]));
}

function normalizeAgent(rawAgent, providers, fallback, index = 0) {
  const defaultPlayer = DEFAULT_PLAYERS[index] || {};
  const providerName = rawAgent.provider || fallback.provider || 'deepseek';
  const provider = providers[providerName];
  if (!provider) {
    throw new Error(`玩家/主持人引用了未解析的 provider：${providerName}`);
  }

  const name = rawAgent.name || fallback.name || `${index + 1}号`;
  const nickname = rawAgent.nickname || fallback.nickname || defaultPlayer.nickname || name;

  return {
    id: Number(rawAgent.id || index + 1),
    name,
    nickname,
    avatar: rawAgent.avatar || fallback.avatar || defaultPlayer.avatar || '',
    ...
    personality: rawAgent.personality || fallback.personality || defaultPlayer.personality || '记录者'
  };
}

function getMissingProviders(config) {
  const agents = [config.host, ...config.players];
...
function normalizeConfig(rawConfig) {
  ...
  const rawPlayers = Array.isArray(rawConfig.players) && rawConfig.players.length
    ? rawConfig.players
    : DEFAULT_PLAYERS;
  ...
  const providers = Object.fromEntries(
    usedProviderNames.map((name) => [name, normalizeProvider(name, rawProviders)])
  );
