...
import { SKIN_PACK_PATH, BUILTIN_TEMPLATE, BASE_INVESTIGATION_QUESTIONS } from './constants';
import type { InvestigationQuestion, SkinTemplate } from './constants';
import { parseSkinMarkdown } from './parser';
import { clone, chooseMemoryExample } from './utils';
...
interface InvestigationQuestionWithPremise extends InvestigationQuestion {
  premise: string;
}

interface MemoryCardResult {
  text: string;
}
...
function getRandomTemplate(rng: () => number = Math.random): SkinTemplate {
  const templates = getSkinTemplates();
  return clone(templates[Math.floor(rng() * templates.length)] || BUILTIN_TEMPLATE);
}

function getInvestigationQuestions(
  template: SkinTemplate = BUILTIN_TEMPLATE,
): InvestigationQuestionWithPremise[] {
...
function getSkinTemplates(): SkinTemplate[] {
  refreshSkinCache();
  return skinCache.templates;
}

function getMarkdownSkinTemplates(): SkinTemplate[] {
  refreshSkinCache();
  return skinCache.templates.filter((template) => template.source !== 'builtin');
}

function refreshSkinCache(): void {
  try {
    ...
    console.error(`皮肤包缓存加载失败，使用内置皮肤：${message}`);
    skinCache = { ...skinCache, templates: [BUILTIN_TEMPLATE] };
  }
}
...
export type { Player, InvestigationQuestionWithPremise };
