...
export function WerewolfPlayerDetailModal({ player, roleVisible, onClose }: WerewolfPlayerDetailModalProps) {
  const roleText = roleVisible ? (ROLE_NAMES[player.roleLabel || ''] || player.roleLabel || ROLE_NAMES[player.role || ''] || '未知身份') : '';
      ...
      <section className="player-detail-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="player-detail-head">
          ...
          <div>
            ...
            {roleVisible && <p className={classNames(dangerRole && 'danger')}>{roleText}</p>}
          </div>
        </div>
        <dl>
          <div><dt>性格</dt><dd>{player.personality || '暂无'}</dd></div>
          {roleVisible && <div><dt>本局身份</dt><dd className={classNames(dangerRole && 'danger')}>{roleText}</dd></div>}
          {roleVisible && <div><dt>身份说明</dt><dd>{getRoleDescription(player, roleVisible)}</dd></div>}
