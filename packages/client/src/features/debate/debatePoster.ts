...
const DEBATE_RESULT_DEBATER_SLOTS: Record<string, (PosterSlot & { roleY: number })[]> = {
  pro: [
...
const DEBATE_RESULT_JUDGE_SLOTS: PosterSlot[] = [
...
export async function createDebateResultPoster(report: DebateShareReport): Promise<string> {
...
async function drawDebateResultPosterBackground(ctx: CanvasRenderingContext2D, width: number, height: number): Promise<void> {
  try {
...
function drawDebateResultPosterFrames(ctx: CanvasRenderingContext2D): void {
...
function drawPosterAvatarPlaceholder(ctx: CanvasRenderingContext2D, slot: PosterSlot, tone: string): void {
...
function drawDebateResultPosterText(ctx: CanvasRenderingContext2D, report: DebateShareReport): void {
...
function drawPosterLineup(ctx: CanvasRenderingContext2D, side: string, players: Player[], nameColor: string, roleColor: string): void {
...
function drawPosterJudges(ctx: CanvasRenderingContext2D, judges: Player[]): void {
...
async function drawDebateResultPosterAvatars(ctx: CanvasRenderingContext2D, report: DebateShareReport): Promise<void> {
...
async function drawCircularAvatar(ctx: CanvasRenderingContext2D, player: Player | undefined, slot: PosterSlot, fallbackTone: string): Promise<void> {
...
function drawAvatarRing(ctx: CanvasRenderingContext2D, slot: PosterSlot, tone: string): void {
...
function drawRotatedPosterBoxText(ctx: CanvasRenderingContext2D, text: string, options: PosterBoxTextOptions): void {
...
function drawPosterBoxText(ctx: CanvasRenderingContext2D, text: string, options: PosterBoxTextOptions): void {
  const {
    x,
    y,
    width,
...
function drawPosterSingleLine(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, width: number, maxSize: number, minSize: number, color: string, weight: string, align = 'center'): void {
  let size = maxSize;
  const clean = cleanPosterText(text);
  ctx.save();
  ctx.textAlign = align as CanvasTextAlign;
...
function truncateCanvasText(ctx: CanvasRenderingContext2D, text: string, width: number): string {
...
function wrapCanvasText(ctx: CanvasRenderingContext2D, text: string, width: number, maxLines: number): string[] {
...
function getPosterPlayerName(player: Player | undefined): string {
  return player?.nickname || player?.name || (player?.id ? `${player.id}号` : '暂未产生');
}

export function downloadResultPoster(dataUrl: string, report: DebateShareReport): void {
  if (!dataUrl) return;
...
function safePosterFileName(value: string): string {
