...
interface RosterProps {
  ...
  sheriffId: string | number | null; candidateIds: Set<number>; withdrawnIds: Set<number>; revealedIdiotIds: Set<number>;
  revealRoles: boolean; viewerPlayerId?: string | number | null;
  onSelect: (player: Player) => void;
}

export function PerspectiveRoster({ players, side, interaction, speakerId, sheriffId, candidateIds, withdrawnIds, revealedIdiotIds, revealRoles, viewerPlayerId, onSelect }: RosterProps) {
  return <section className={classNames('perspective-roster', `perspective-roster--${side}`)} aria-label={`${side === 'left' ? '左' : '右'}侧玩家席位`}>
    {players.map((player, index) => {
      ...
      const sheriff = id === Number(sheriffId);
      const candidate = candidateIds.has(id) && !withdrawnIds.has(id);
      const withdrawn = withdrawnIds.has(id);
      const revealedIdiot = revealedIdiotIds.has(id);
      const showRole = revealRoles || own;
      const status = speaking ? '发言中' : sheriff ? '警长' : withdrawn ? '已退水' : candidate ? '警长候选' : revealedIdiot ? '已翻牌' : target ? '行动目标' : actor ? '正在行动' : !player.alive ? '已出局' : '';
      const statusKind = speaking ? 'speaking' : sheriff ? 'sheriff' : withdrawn ? 'withdrawn' : candidate ? 'candidate' : revealedIdiot ? 'revealed' : target ? 'target' : actor ? 'acting' : !player.alive ? 'dead' : '';
      const StatusIcon = speaking ? Mic2 : sheriff ? Crown : candidate ? Users : revealedIdiot ? Eye : !player.alive ? Skull : null;
      return <button type="button" className={classNames('perspective-seat', speaking && 'is-speaking', actor && 'is-acting', target && 'is-target', sheriff && 'is-sheriff', candidate && 'is-candidate', withdrawn && 'is-withdrawn', revealedIdiot && 'is-revealed', own && 'is-viewer', !player.alive && 'is-dead')} onClick={() => onSelect(player)} key={player.id}>
        ...
        {status && <i data-status={statusKind}>{StatusIcon && <StatusIcon size={12} />}{status}</i>}
      </button>;
    })}
  </section>;
}

export function InteractionStage({ interaction, players, view, round }: { interaction: WerewolfInteractionState; players: Player[]; speech: SpeechState | null; view: 'god' | 'player'; round: WerewolfRound | null }) {
  ...
  const showDetails = shouldShowWerewolfStageDetails(interaction);
  const animationKey = getWerewolfInteractionAnimationKey(interaction);
  return <main className={classNames('interaction-stage', night && 'is-night')} data-tone={interaction.tone} data-template={interaction.template} data-action={interaction.action} data-status={interaction.status}>
    <section className="interaction-stage__card" aria-live="polite" key={animationKey}>
      ...
      {showDetails && view === 'god' && round?.phase === 'day' && <div className="interaction-stage__facts"><span>已投票 <b>{Object.keys(round.votes || {}).length}</b></span><span>警长 <b>{round.sheriffId ? formatWerewolfSeatLabel(round.sheriffId, players) : '未产生'}</b></span></div>}
    </section>
  </main>;
}
