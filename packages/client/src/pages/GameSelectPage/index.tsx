...
export function GameSelectPage({ onStartDebate, onStartWerewolf, onReplayGame }: GameSelectPageProps) {
  ...
  const [recentGames, setRecentGames] = useState<Record<string, Record<string, unknown>[]>>({});
          ...
          return (
            <article className={`game-entry-card ${game.tone}`} key={game.key}>
              ...
              <RecentGameList
                ...
                onOpen={(historyGame) => onReplayGame?.(game.key, historyGame.id as string, selectedIds)}
              />
            </article>
          );
        })}
...
function RecentGameList({ gameType, games, onOpen }: RecentGameListProps) {
  return (
    <div className="game-recent-list">
      <strong>最近对局</strong>
      {games.length ? games.map((game) => (
        <button type="button" key={String(game.id)} onClick={() => onOpen(game)}>
          <span>{getHistoryTitle(gameType, game)}</span>
          <em>{formatTime(game.createdAt as string)}</em>
        </button>
      )) : <p>暂无历史对局。</p>}
    </div>
  );
...
interface GamePlayerEditorProps {
  gameKey: string;
  players: Player[];
  draftIds: number[];
