...
import type { WerewolfMode, WerewolfRole } from '../../types/entities';
import type { FilterState } from '../../types/api';

export function WerewolfModeManager() {
  const { message } = AntApp.useApp();
  const [modes, setModes] = useState<WerewolfMode[]>([]);
  const [roles, setRoles] = useState<WerewolfRole[]>([]);
  const [editing, setEditing] = useState<WerewolfMode | null>(null);
  const [filters, setFilters] = useState<FilterState>({});

  useEffect(() => {
    Promise.all([
      adminRequest<WerewolfMode[]>('/werewolf-modes'),
      adminRequest<WerewolfRole[]>('/werewolf-roles')
    ]).then(([m, r]) => {
      setModes(m);
      setRoles(r);
    }).catch(() => {});
  ...
  return (
    <>
      <Card title="狼人杀模式" extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setEditing({ roles: [], sheriff: { enabled: true, firstDayElection: true, voteWeight: 1.5 }, winCondition: 'side' } as unknown as WerewolfMode)}>新增模式</Button>}>
        <ListFilterBar
          value={filters}
          onChange={setFilters}
        ...
        <Table rowKey="id" dataSource={filteredModes} columns={[
          ...
          { title: '胜利条件', dataIndex: 'winCondition', render: (value: string) => WEREWOLF_WIN_OPTIONS.find((item) => item.value === value)?.label || value },
          { title: '阵容', dataIndex: 'roles', render: (items: WerewolfMode['roles']) => summarizeWerewolfRoles(items, roles) },
          { title: '说明', dataIndex: 'description', ellipsis: true },
          { title: '启用', dataIndex: 'enabled', render: enabledTag },
          { title: '操作', width: 150, render: (_: unknown, record: WerewolfMode) => <TableActions onEdit={() => setEditing(record)} onDelete={() => remove(record.id)} /> }
        ]} />
      </Card>
...
interface WerewolfModeModalProps {
  open: boolean;
  initialValues: WerewolfMode | null;
  roles: WerewolfRole[];
  onCancel: () => void;
  onSave: (values: Record<string, unknown>) => void;
}

function WerewolfModeModal({ open, initialValues, roles = [], onCancel, onSave }: WerewolfModeModalProps) {
