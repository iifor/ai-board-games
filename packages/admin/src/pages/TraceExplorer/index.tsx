...
const STATUS_MAP: Record<string, string> = { recording: '进行中', completed: '已完成', error: '错误' };
...
export function TraceExplorer() {
  ...
  const fetchTraces = useCallback(async () => {
    ...
    try {
      ...
      const data = await adminRequest<Trace[]>(`/traces?${params.toString()}`);
  ...
  function confirmRemove(trace: Trace) {
    Modal.confirm({
      title: '删除观测数据',
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (type: string) => <Tag>{GAME_LABELS[type] || type}</Tag>
    },
    {
      title: '状态', dataIndex: 'status', key: 'status', width: 100,
      render: (s: string) => <Tag color={STATUS_COLORS[s] || 'default'}>{STATUS_MAP[s] || s}</Tag>
    },
