...
import type { Trace, LlmCall, AgentDecision, TraceSnapshot } from '../../types/trace';

const { Title, Text, Paragraph } = Typography;

interface PlayerTraceData {
  trace: Trace;
  llmCalls: LlmCall[];
...
export function AgentTraceView() {
  ...
  const [data, setData] = useState<PlayerTraceData | null>(null);
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const result = await adminRequest<PlayerTraceData>(`/traces/${id}/player/${playerId}`);
        setData(result);
      } catch { setData(null); }
  ...
  const { trace, llmCalls = [], decisions = [], snapshots = [] } = data || {} as PlayerTraceData;
  ...
  const invalidVotes = voteDecisions.filter((d) => {
    ...
    const snap = [...snapshots].reverse().find((s) => {
      try {
        const state = typeof s.snapshot_json === 'string' ? JSON.parse(s.snapshot_json) as { players?: Array<{ id: string; alive: boolean }> } : s.snapshot_json as { players?: Array<{ id: string; alive: boolean }> };
        return state.players && state.players.some((p) => String(p.id) === String(d.chosen_target));
      } catch { return false; }
    });
    if (!snap) return false;
    try {
      const state = typeof snap.snapshot_json === 'string' ? JSON.parse(snap.snapshot_json) as { players?: Array<{ id: string; alive: boolean }> } : snap.snapshot_json as { players?: Array<{ id: string; alive: boolean }> };
  ...
  if (playerLlmCalls.length) {
    try {
      const msgs = typeof playerLlmCalls[0].messages_json === 'string'
        ? JSON.parse(playerLlmCalls[0].messages_json) as Array<{ role: string; content: string }>
    ...
    <div>
      ...
      <Card loading={loading}>
        ...
        <Table
          ...
          columns={[
            { title: '时间', dataIndex: 'created_at', key: 'time', width: 180, render: (t: string) => t ? new Date(t).toLocaleString() : '-' },
            { title: '决策类型', dataIndex: 'decision_type', key: 'type', width: 120, render: (t: string) => <Tag>{t}</Tag> },
            { title: 'Phase', dataIndex: 'phase', key: 'phase', width: 80 },
            { title: 'Day', dataIndex: 'day', key: 'day', width: 60 },
            { title: '目标', dataIndex: 'chosen_target', key: 'target', width: 60 },
            {
              title: '输出摘要', dataIndex: 'response_text', key: 'response',
              render: (t: string) => t ? <Text ellipsis style={{ maxWidth: 300 }}>{t.slice(0, 80)}</Text> : '-'
            },
            { title: '回退', dataIndex: 'fallback_used', key: 'fallback', width: 60, render: (v: boolean) => v ? <Tag color="orange">是</Tag> : '' },
          ]}
