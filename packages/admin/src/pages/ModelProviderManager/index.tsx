...
import type { ModelProvider } from '../../types/entities';
...
export function ModelProviderManager() {
  ...
  async function refresh() {
    try {
      setProviders(await adminRequest<ModelProvider[]>('/model-providers'));
    } catch (error) {
  ...
  function confirmRemove(provider: ModelProvider) {
    Modal.confirm({
      title: '删除供应商',
  ...
  return (
    <>
      <Card title="供应商列表" extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setEditing({} as ModelProvider)}>新增供应商</Button>}>
        <ListFilterBar
          value={filters}
          onChange={setFilters}
        ...
        <Table rowKey="id" dataSource={filteredProviders} columns={[
          ...
          { title: 'API Key', dataIndex: 'apiKey', ellipsis: true, render: (value: string) => value || '-' },
...
interface ProviderModalProps {
  open: boolean;
  initialValues: ModelProvider | null;
  onCancel: () => void;
  onSave: (values: Record<string, unknown>) => void;
}

function ProviderModal({ open, initialValues, onCancel, onSave }: ProviderModalProps) {
