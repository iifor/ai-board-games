...
All role-password files are host-provided Compose secrets and remain outside Git. The bootstrap account is used only to create `consensus_app` and `consensus_migrator`; bootstrap credentials are not supplied to either application image. On first PGDATA initialization the role script rejects missing or empty role-password files, creates SCRAM credentials, and installs `hostnossl ... reject` plus `hostssl ... scram-sha-256` rules. The migration role owns the `consensus` schema; the app role receives only connection, schema usage, table DML, and sequence privileges.

TLS files are host-provided bind mounts, never generated by this repository. The leaf certificate must contain `DNS:postgres` in its SAN because Compose clients use the hostname `postgres` with `verify-full`. On Linux, make the private key readable only by the PostgreSQL Alpine container user (`70:70`, mode `0600`); use `70:70`, mode `0644` for the certificate and CA. Store the secret files in a root-owned `0700` directory and mount them through Compose secrets; the entrypoint and initialization script require readable non-empty files, so the mounted secret targets are read-only and readable by the PostgreSQL process only for initialization. The app and migrator mount only the CA at `/run/postgres-ca/ca.crt`.
...
```powershell
docker build --progress=plain -t consensus-runtime-check .
docker run --rm consensus-runtime-check node -e "const fs=require('node:fs'); const forbidden=['/app/packages/db-migrator','/app/artifacts','/app/.superpowers','/app/.worktrees']; if(forbidden.some(fs.existsSync)) process.exit(1); try { require.resolve('better-sqlite3'); process.exit(1) } catch { process.exit(0) }" # packages/db-migrator 一次性迁移 dependency isolation probe
```
