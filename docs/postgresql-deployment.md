...
上线前必须有数据库可用性、复制/WAL 归档、备份新鲜度、连接池、超时、锁/死锁、慢查询、autovacuum、表/索引膨胀、磁盘、证书到期和 `/api/toc/health` 告警。发布聚合器保持恰好 16 个 final gates，固定检查 CI、无关键 skip、执行备份、restore drill、两次独立演练、同源 hash、runtime 无旧数据库依赖、TLS、最小权限、pool/timeouts、三份同 schema smoke（两份演练、一份生产 cutover）、唯一完整的 `production.cutover` 闭环和独立 operator signoff。文档真相继续由签名 CI/environment evidence 覆盖，不再占用独立 gate。
...
## Execute-gated production cutover contract (2026-08-11)

The offline `cutover` command consumes only a complete verified backup manifest and its exact `sqlite-consistent.sqlite`; it never opens the raw/live SQLite source. Without `--execute`, it performs stable local verification only and creates no output. Execute requires a separate bounded v1 authorization whose run ID, release candidate, manifest hash, snapshot hash, maintenance window, fixed target, and three distinct ordered approvers all match. Missing, malformed, stale, future, placeholder, or mismatched authorization fails before database access or output reservation. This authorization permits migration phases only and never authorizes traffic, app startup, or nginx startup.

The ops entrypoint remains the only source of the fixed migrator connection. Cutover requires a queryless `DATABASE_URL`, `RELEASE_CANDIDATE_SHA`, `DATABASE_SSL=verify-full`, and a readable `DATABASE_CA_PATH`; target database/schema/role/host/port are not CLI choices, and TLS is supplied only as explicit CA-backed client options with `rejectUnauthorized: true`. After local verification it creates a non-overwriting permanent owner receipt, copies only the small verified authorization and manifest bytes, and acquires one global session advisory lock. It opens the verified consistent snapshot once as a readonly/query-only SQLite handle, rechecks file identity and SHA-256 around import and validation, and never reopens an unbound path. The same PostgreSQL client remains open through the PostgreSQL 16 identity/privilege/TLS/empty-target gate, compiled canonical schema migration, transactional import, CA-backed formal validation, production-purpose compiled application smoke, and final evidence report. Any failure preserves the schema and every published artifact, suppresses dependent phases, and never retries, drops, truncates, or deletes evidence.

The final cutover report contains only fixed target assertions, source/manifest/authorization/candidate hashes, phase checks, relative artifact paths and hashes, fixed errors, and duration. It excludes connection URLs, passwords, endpoints, host source paths, source rows, and raw filesystem/driver errors; durable migration evidence uses the fixed source marker `[verified-consistent-snapshot]`. A passed report binds a precomputed immutable completion receipt, but that receipt is published only after advisory unlock and connection close both succeed. Release readiness accepts exactly one passed cutover report plus its completion receipt, matching same-run/same-schema validation and production smoke reports, and verifies the exact hashes and paths of authorization, manifest, owner, completion, migration, validation, and smoke artifacts; swapping any artifact or mixing runs/schemas fails closed. The signoff manifest must cover every report and artifact exactly once. Validation's exact intentional skipped-table checks are accepted, while unknown or required skips still fail and `tests.no-critical-skips` remains signed. The final gate count remains exactly 16.

Production-purpose smoke is observationally clean: it uses uniquely run-owned synthetic players, memories, game state, and observability rows and removes every synthetic row in `finally` after both success and failure. Pre-existing players and memories are never selected for mutation and remain byte-for-byte unchanged.
...
Production Compose defaults to the private `postgres:16-alpine` service only, with the named durable volume `consensus-postgres-data` and no PostgreSQL host port. `app` is gated by the explicit `application` profile, waits for database health, and connects only as `consensus_app`; `nginx` is gated by the separate `traffic` profile. The app's fixed production settings are `DATABASE_SSL=verify-full`, schema `consensus`, pool maximum `10`, connection timeout `5000`, and statement timeout `30000`. The offline `migrator` is a separate image under the `ops` profile and connects only as `consensus_migrator`. Production operators use the reviewed wrappers; an ordinary default Compose invocation cannot start app, nginx, or migrator.
...
## Linux first-deployment execution

Follow `docs/runbooks/postgresql-first-deployment-cutover.md` for the first production deployment. The POSIX wrappers clear ambient Compose file/project/profile/env-file/removal overrides and use only the canonical repository Compose file/project with explicit `application`, `traffic`, and `ops` profiles. Production runtime builds use a named `application_source` additional context that must be an independent, detached, exact, clean candidate checkout; only the reviewed start wrapper overlays tooling. The Dockerfile embeds a path-and-Git-blob input manifest computed from the actual context. That manifest deliberately excludes host filesystem execute bits because Windows BuildKit rewrites them; the separately verified candidate Git tree remains authoritative for Git modes and symlink identity. The build wrapper independently recomputes the candidate tree and manifest, then the compiled recorder atomically creates a non-overwriting typed build receipt binding those values, the stable manifest artifact, tooling HEAD, and both image digests. Host and traffic gates recompute from the independent checkout again and cross-check the receipt, artifact, and current runtime marker. OCI labels remain a secondary check: development may leave them `unbound`, but production gates reject unbound/wrong values. Credentials and CA/key bytes stay in mounted files or child-process environment and never enter operator argv/output. A typed freeze receipt and approved maintenance artifact are revalidated before backup, preflight, and the single cutover attempt; backup, cutover, completion, release, and traffic evidence carry one exact freeze SHA. `start-app-only.sh` cannot activate nginx. `start-nginx-gated.sh` alone activates the `traffic` profile after all gates pass.

Traffic opening and observation are separate decisions. The traffic authorization is manually created and expires; its verifier is read-only. A passed release report carries a replayable `evidenceClosure` binding the exact operator signoff and every signed input report by relative path, size, and SHA-256. Before accepting 16/16, the traffic verifier reloads that signoff/report manifest and all report artifacts with the same release evidence loader, then independently reproduces the gate result, maintenance window, and freeze hash; a bare self-declared 16/16 report is invalid. Operators retain the nginx-only transcript and conservatively record canonical `trafficOpenedAt` immediately after the command succeeds. The observation receipt is also manually recorded, must carry that typed opening time, cannot predate traffic approval/opening, and is independently verified after at least 60 minutes plus a real new PostgreSQL backup restore test. If PostgreSQL accepted any new business write, SQLite rollback is forbidden; operators must close traffic, preserve PostgreSQL, and choose forward repair or a separately designed reconciliation project.
