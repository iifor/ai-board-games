...
The Step 10 runtime image evidence is invalid unless the Docker build log shows a bounded `load build context` transfer and the final-image probe exits zero. The repository `.dockerignore` must exclude operational `artifacts/`, `.superpowers/`, `.worktrees/`, backup/report/log/temp outputs, and all SQLite/database or dump/backup suffixes without assuming lowercase. Any basename ending in `-wal`, `-shm`, or `-journal` is also excluded. Do not delete the legacy source set to satisfy this gate; `packages/data/*.sqlite*` remains a rollback/migration input outside the Docker context.
...
## Handoff to the first production deployment

This readiness runbook does not execute the real cutover or authorize traffic. After its independent approval, use `postgresql-first-deployment-cutover.md` from Step 1 with a new production evidence root. Do not skip directly to nginx: exact candidate/tooling/role image provenance, typed freeze receipt and approved maintenance artifact, production cutover, app-only evidence, the exact 16/16 release report, a separate manually authored traffic authorization, manual nginx start/openedAt transcript, and the independent 60-minute observation receipt are non-exchangeable gates.
