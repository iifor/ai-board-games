...
Production smoke is closed by default. The compiled adapter accepts database `consensus` only with the internal `production-cutover` purpose, exact `consensus_migrator@postgres:5432/consensus` identity, schema `consensus`, a queryless URL, and inherited `DATABASE_SSL=verify-full` plus a readable `DATABASE_CA_PATH`; it constructs explicit `ssl: { ca, rejectUnauthorized: true }` options itself. Ordinary smoke still requires a dedicated `_test` or `_rehearsal` database. This purpose is internal to the offline cutover process and does not authorize public traffic or start nginx. Production smoke creates only run-owned synthetic players, memories, game state, and observability rows, then removes all of them in `finally` on both success and failure; it never updates or deletes pre-existing business rows.
...
`cutover` 是独立的 production-only 编排命令。它只接受已验证 backup 中的 `sqlite-consistent.sqlite` 与 manifest；无 `--execute` 时只稳定复核本地证据，零数据库调用且不创建 output。execute 必须提供单独的 v1 授权文件，并从 ops 子进程环境读取固定、无 query 的 migrator URL、`RELEASE_CANDIDATE_SHA`、`DATABASE_SSL=verify-full` 与可读 `DATABASE_CA_PATH`；`--target` 及 operator 提供的 database/schema/role/host/port 参数都会在 I/O 前拒绝。授权校验、固定目标/TLS 本地校验通过后，工具以永久 owner receipt 排他预留 run，稳定捕获 snapshot 后打开同一个 readonly/query-only SQLite handle，复核文件身份和 hash，并持有该 handle 贯穿事务导入与 validation 前后复核；永久 migration evidence 仅记录固定占位来源，不泄露主机绝对路径。工具同时使用同一个 `pg.Client` advisory-lock session 贯穿只读目标门禁、compiled canonical migration adapter、事务导入、formal validation、production-purpose compiled smoke 和最终报告写入。passed report 在锁内引用预计算 completion receipt hash；只有 advisory unlock 与 session close 成功后才原子发布 receipt，release readiness 会校验 authorization/manifest/owner/completion/migration/validation/smoke 的完整同 run、同 schema、同 hash 闭包。失败不 retry/drop/truncate，不删除 schema 或证据；该授权不启动 app/nginx，也不构成 traffic approval。
