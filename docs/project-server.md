...
正式 SQLite backup 将完整 runId 保留在最终目录、报告和 manifest 中，但 staging、failed site 与 reservation owner token 只使用固定长度摘要及排他随机后缀。Windows 实际 recovery/consistent/final SQLite 路径在任何源内容复制与 SQLite-open 前执行预算检查；超限固定返回脱敏的 `BACKUP_PATH_TOO_LONG`，不创建部分最终 backup，也不以缩短合法 runId 作为运维前提。
