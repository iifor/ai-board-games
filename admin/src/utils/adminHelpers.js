...
export function modelName(player, models) {
...
export function uniqueOptions(values, formatter = (value) => value) {
  const seen = new Set();
  return values
...
export function summarizeWerewolfRoles(items = [], roles = []) {
...
export function normalizeSkinFormValues(values) {
...
export function formatVoiceProvider(value) {
  if (value === 'azure') return 'Azure Speech';
  if (value === 'browser') return '浏览器本地语音';
  return value || '-';
}

export function formatGameMode(value, game, modes = []) {
...
export function formatWinner(value) {
  const map = {
    pro: '正方', con: '反方',
...
export function formatSideOrCamp(record, gameType) {
...
export function formatRole(record, gameType) {
...
export function getImportPromptRule(gameType) {
  if (gameType === 'debate') {
    return [
...
export function buildImportGenerationPrompt(gameType, players = []) {
...
export async function playVoicePackage(voice, text, audioRef) {
