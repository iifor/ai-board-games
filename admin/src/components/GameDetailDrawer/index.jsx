...
import { formatTime, formatGameMode, formatWinner, formatSideOrCamp, formatRole, getGameTitle } from '../../utils/adminHelpers';
import { exportJsonFile, safeFilenamePart } from '../../utils/fileExport';

const { Text, Title } = Typography;

export function GameDetailDrawer({ game, onClose, onDelete }) {
  const [modes, setModes] = useState([]);

  useEffect(() => {
    if (!game || game.gameType !== 'werewolf') {
      setModes([]);
      return;
    }
    adminRequest('/werewolf-modes').then(setModes).catch(() => {});
  }, [game]);
        ...
        <Space direction="vertical" size={16} className="admin-full">
          ...
          {game.gameType === 'werewolf' && game.event?.werewolfMode && (
            <Card size="small" title={`模式：${game.event.werewolfMode.name || game.modeName || '-'}`}>
              ...
              {game.event.werewolfMode.roles?.length > 0 && (
                <>
            ...
            <Table
              ...
              columns={[
                ...
                ...(game.gameType === 'debate'
                  ? [
                      { title: '立场', render: (_, record) => formatSideOrCamp(record, game.gameType) },
                      { title: '辩位', render: (_, record) => formatRole(record, game.gameType) }
                    ]
                  : [
                      { title: '阵营', render: (_, record) => formatSideOrCamp(record, game.gameType) },
