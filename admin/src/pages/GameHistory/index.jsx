...
export function GameHistory({ gameType }) {
  ...
  useEffect(() => {
    setLoading(true);
    const fetches = [adminRequest(`/games?gameType=${gameType}`), adminRequest('/players')];
    if (gameType === 'werewolf') fetches.push(adminRequest('/werewolf-modes'));
    Promise.all(fetches)
      .then(([g, p, m]) => {
        setGames(g);
        setPlayers(p);
  ...
  async function refresh() {
    ...
    try {
      const fetches = [adminRequest(`/games?gameType=${gameType}`), adminRequest('/players')];
      if (gameType === 'werewolf') fetches.push(adminRequest('/werewolf-modes'));
      const [g, p, m] = await Promise.all(fetches);
      setGames(g);
      setPlayers(p);
      if (m) setWerewolfModes(m);
    } finally {
...
function ImportGameModal({ open, gameType, players = [], onCancel, onImported }) {
  const { message } = AntApp.useApp();
        ...
        { title: '时间', dataIndex: 'createdAt', render: formatTime },
        {
          ...
          render: (_, record) => (
            <Space>
              ...
              <TableActions onEdit={() => onOpen?.(record.id)} editText="详情" onDelete={onDelete ? () => onDelete(record.id) : null} />
            </Space>
          )
        }
      ]}
