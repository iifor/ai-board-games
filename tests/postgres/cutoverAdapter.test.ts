...
const TLS_ENVIRONMENT = productionTlsEnvironment('/run/secrets/postgres_ca.crt');
...
test('compiled cutover adapter fails closed for wrong input, target identity, or existing schema without migrate', async () => {
  for (const candidate of [
    ...
    { ...REQUEST, targetUrl: productionDatabaseUrl({ role: 'consensus_app' }) },
    { ...REQUEST, extra: true },
  ]) {
    assert.throws(
      () => parseRequest(JSON.stringify(candidate), TLS_ENVIRONMENT),
  ...
  await assert.rejects(executeRequest(REQUEST, {
    ...
    readCa: async () => 'test-ca',
  }, TLS_ENVIRONMENT), (error: unknown) => (error as { code?: string }).code === 'CUTOVER_TARGET_UNSAFE');
...
test('compiled cutover adapter preserves a primary failure when close also fails', async () => {
  ...
  }, TLS_ENVIRONMENT), (error: unknown) => error === primary);
});
